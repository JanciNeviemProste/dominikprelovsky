...
// Server-side upload cez náš endpoint (same-origin → žiadne CORS). Vercel serverless
// request body má ~4,5 MB strop, takže cez tlačidlo idú len malé PDF. Väčšie súbory
// (napr. 30 MB) sa nahrajú vo Vercel dashboarde (Storage → Blob) a ich URL sa vloží
// do poľa „PDF URL" v admine. (Client-side direct upload sme nepoužili — @vercel/blob
// client PUT-uje na vercel.com/api/blob, čo z prehliadača padá na CORS.)
...
export async function POST(req: NextRequest) {
  ...
  if (file.size > MAX_BYTES) {
    return NextResponse.json(
      {
        error: `Súbor je príliš veľký (max ${MAX_BYTES / 1024 / 1024} MB cez tlačidlo). Nahraj ho vo Vercel dashboarde (Storage → Blob) a jeho URL vlož do poľa „PDF URL".`,
      },
      { status: 400 },
    );
  }
